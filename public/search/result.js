...
$(function() {
    ...
    var searchFirst = function() {
        ...
        $.ajax({
            ...
            var n = count % 20 == 0 ? 0 : 1;
            // 分页
            $('#page-index').Page({
                totalPages: parseInt((+count)/20) + n, //分页总数
                liNums: 9, //分页的数字按钮数(建议取奇数)
            ...
            // 跳转页
            $('#page-index').prepend('<div id="jump-page"><input type="text" class="jump-num"><button class="jump-to">跳转</button></div>');

            $('#page-index').on('click', '.jump-to', function() {
                var pageNum = $('.jump-num').val();
    ...
    // 绑定查询事件
    $('tbody').on('click', '#wooyun-type', function() {
        location.href = '/routes/result.js?type=type&content=' + $(this).text();
    });

    $('tbody').on('click', '#wooyun-author', function() {
        location.href = '/routes/result.js?type=author&content=' + $(this).text();
    });
